...
from SerkonAnomalib.deploy.export import CompressionType, ExportType, InferenceModel
...
class ExportMixin:
    ...
    def _create_export_root(self,export_root: str | Path) -> Path:
    ...
    def to_torch(
        ...
        Args:
            ...
            export_file_name(Path): Name of the output file.
        ...
        Examples:
            ...
            pt_model_path ---> export_root / export_file_name.pt
        ...
        print(f"[LOG] Export root is --> {export_root}")
        export_root = self._create_export_root(export_root)
        metadata = self._get_metadata(task=task)
        pt_model_path = export_root / f"{export_file_name}.pt"
        torch.save(
...
def create_export_root(export_root: str | Path, export_type: str) -> Path:
    """Create export directory.
